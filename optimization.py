...
def mask_gradient(grad, keep_grad, num_freeze_steps=0, name="mask_gradient"):
    with tf.name_scope(name, "mask_gradient", [grad]) as scope:
        if grad is None:
            return grad

        values = tf.convert_to_tensor(
            grad.values if isinstance(grad, tf.IndexedSlices) else grad, name="grad")
        global_step = tf.compat.v1.train.get_global_step()
        is_mask = tf.cast(global_step < num_freeze_steps, dtype=values.dtype)
        gating = tf.cast(keep_grad, dtype=values.dtype)
        is_mask = is_mask * (1.0 - gating)

        # if global_step > num_freeze_steps or keep_grad:
        #     masked_values = values
        # else:
        #     # masked_values = tf.zeros_like(values)
        #     masked_values = tf.constant(0.0)

        with tf.colocate_with(grad):
            masked_values = (1.0 - is_mask) * values

        if isinstance(grad, tf.IndexedSlices):
          return tf.IndexedSlices(masked_values, grad.indices, grad.dense_shape)
...
def create_optimizer(loss, init_lr, num_train_steps, num_warmup_steps, hvd=None, manual_fp16=False, use_fp16=False, num_accumulation_steps=1,
                     optimizer_type="adam",
                     allreduce_post_accumulation=False, spectral_reg_rate=0.0,
                     init_loss_scale=2**32,
                     non_freeze_scope=None,
                     num_freeze_steps=0):
  ...
  if optimizer_type == "lamb":
      ...
      optimizer = AdamWeightDecayOptimizer(
          learning_rate=learning_rate,
          weight_decay_rate=0.01,
          spectral_reg_rate=spectral_reg_rate,
          beta_1=0.9,
          beta_2=0.999,
          epsilon=1e-6,
          exclude_from_weight_decay=["LayerNorm", "layer_norm", "bias"],
          exclude_from_spectral_reg=["LayerNorm", "layer_norm", "bias", "embeddings", "cls"],
      )
  ...
  gating_gradient = False
  if num_freeze_steps > 0 and non_freeze_scope:
    non_freeze_vars = sum([
        tf.get_collection(
            tf.GraphKeys.TRAINABLE_VARIABLES, scope)
        for scope in non_freeze_scope], [])
    tf.logging.info("Only training the following variables for %d steps:" %
                    num_freeze_steps)
    non_freeze_var_names = [
        vv.name
        for vv in non_freeze_vars
    ]
    tf.logging.info("\n".join(non_freeze_var_names))
    gating_gradient = True
  else:
    non_freeze_var_names = [
        vv.name
        for vv in tvars
  ...
  non_freeze_var_names = set(non_freeze_var_names)
  ...
  if num_accumulation_steps > 1:
      ...
      def update(accum_vars):
          ...
          if gating_gradient:
            update_grads_and_vars = [
                (mask_gradient(gg, vv.name in non_freeze_var_names,
                               num_freeze_steps=num_freeze_steps), vv)
                for gg, vv in zip(accum_vars, tvars)]
          else:
            update_grads_and_vars = list(zip(accum_vars, tvars))

          # return optimizer.apply_gradients(list(zip(accum_vars, tvars)), global_step=global_step)
          return optimizer.apply_gradients(update_grads_and_vars, global_step=global_step)
      ...
      if gating_gradient:
        update_grads_and_vars = [
            (mask_gradient(gg, vv.name in non_freeze_var_names,
                           num_freeze_steps=num_freeze_steps), vv)
            for gg, vv in zip(clipped_grads, tvars)]
      else:
        update_grads_and_vars = list(zip(clipped_grads, tvars))

      train_op = optimizer.apply_gradients(
          update_grads_and_vars, global_step=global_step)
...
class AdamWeightDecayOptimizer(tf.train.Optimizer):
  ...
  def __init__(self,
               ...
               epsilon=1e-6,
               exclude_from_weight_decay=None,
               exclude_from_spectral_reg=None,
    ...
    self.epsilon = epsilon
    self.exclude_from_weight_decay = exclude_from_weight_decay
    self.spectral_reg_rate = spectral_reg_rate
    self.exclude_from_spectral_reg = exclude_from_spectral_reg
  ...
  def apply_gradients(self, grads_and_vars, global_step=None, name=None,
    ...
    for (grad, param) in grads_and_vars:
      ...
      # beta = self.spectral_reg_rate
      # if beta > 0 and self._do_use_spectral_reg(param_name):
      #   spectral_update = beta * tf.matmul(
      #       tf.matmul(next_param, next_param, transpose_b=True), next_param) - (
      #           beta * next_param)
      #   next_param = next_param - self.learning_rate * spectral_update
  ...
  def _do_use_spectral_reg(self, param_name):
    """Whether to use spectral regularization weight decay for `param_name`."""
    if not self.weight_decay_rate:
      return False
    if self.exclude_from_spectral_reg:
      for r in self.exclude_from_spectral_reg:
        if re.search(r, param_name) is not None:
          return False
    return True
