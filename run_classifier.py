...
import random
import modeling
# import modeling_rec as modeling
import optimization
import tokenization
...
from mnli_utils.loss_helper import (
  compute_margin_loss,
  compute_vat_loss,
  compute_double_forward_loss_w_add_noise,
  compute_approx_jacobian_norm,
  compute_weighted_jacobian_norm,
...
# Margin loss related parameters.
flags.DEFINE_float(
    "tau", 12,
    "Upper bound for the margin, prop to the model Lipschitz constant.")

flags.DEFINE_float(
    "margin_reg_rate", 0.0,
    "Weight for controlling the margin loss.")

flags.DEFINE_float("spectral_reg_rate", 0.0,
                   "Spectral regularizer rate for Adam.")

flags.DEFINE_float(
    "jacobian_reg_rate", 0.0,
    "Weight for controlling the jacobian regularization loss.")
...
flags.DEFINE_string(
    "noise_type", "tok",
    "String for the noise type {tok, seq, tok+seq}"
)
...
flags.DEFINE_float(
    ...
    "Weight for controlling the output layer dropout keep prob.")
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
  ...
  output_shape = modeling.get_shape_list(output_layer, expected_rank=2)
  batch_size = output_shape[0]
  hidden_size = output_shape[1]

  logits = compute_forward_logits(output_layer, num_labels,
                                  is_training=is_training,
                                  output_layer_dropout=FLAGS.output_layer_dropout,
                                  reuse=False)
  ...
  # Adds margin loss.
  if FLAGS.margin_reg_rate > 0.0 and is_training:
    margin_loss, _ = compute_margin_loss(
        logits, one_hot_labels, tau=FLAGS.tau)
    loss += FLAGS.margin_reg_rate * margin_loss
  else:
    margin_loss = tf.constant(0.0)

  # Adds jacobian reg loss.
  if FLAGS.jacobian_reg_rate > 0.0 and is_training:
    tf.logging.info("Using weighted Jacobian regularization")
    jacobian_loss = compute_approx_jacobian_norm(
        logits, model.get_embedding_output())

    loss += FLAGS.jacobian_reg_rate * jacobian_loss
  else:
    jacobian_loss = tf.constant(0.0)

  if FLAGS.double_forward_reg_rate > 0.0 and is_training:
    ...
    loss += FLAGS.double_forward_reg_rate * double_forward_loss
  else:
    double_forward_loss = tf.constant(0.0)

  if FLAGS.vat_reg_rate > 0.0 and is_training:
    vat_type_loss_type = FLAGS.double_forward_loss.split("-")
    vat_type = "vanilla"
    loss_type = FLAGS.double_forward_loss
    if len(vat_type_loss_type) > 1:
        vat_type = vat_type_loss_type[0]
        loss_type = vat_type_loss_type[1]

    tf.logging.info("Using vanilla VAT loss for training")
    ...
    loss += FLAGS.vat_reg_rate * vat_loss

  else:
    vat_loss = tf.constant(0.0)

  return (loss, per_example_loss, logits, probabilities, margin_loss,
          jacobian_loss, double_forward_loss, vat_loss)
...
def model_fn_builder(task_name, bert_config, num_labels, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    def metric_fn(per_example_loss, label_ids, logits):
        ...
        if task_name == "hans":
            tf.logging.info("For hans, maps prediction of neutral to contradict.")
            # neural_index = 2
            # mask = 1.0 - tf.one_hot([neural_index], depth=3, dtype=tf.float32)
            # predictions = tf.argmax(logits + tf.log(mask), axis=-1, output_type=tf.int32)
            mask = tf.cast(predictions < 2, dtype=tf.int32)
    ...
    if not is_training and FLAGS.use_trt:
        ...
        return output_spec
    (total_loss, per_example_loss, logits, probabilities, margin_loss,
     jacobian_loss, double_forward_loss, vat_loss) = create_model(
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      logging_hook = tf.train.LoggingTensorHook(
          {"total_loss": total_loss, "clean_loss":
           ...
           "scaled_margin_loss": FLAGS.margin_reg_rate * margin_loss,
      ...
      train_op = optimization.create_optimizer(
          ...
          spectral_reg_rate=FLAGS.spectral_reg_rate,
      )
...
def main(_):
  ...
  tf.logging.info("margin_reg_rate: %f" % FLAGS.margin_reg_rate)
  tf.logging.info("spectral_reg_rate: %f" % FLAGS.spectral_reg_rate)
  tf.logging.info("jacobian_reg_rate: %f" % FLAGS.jacobian_reg_rate)
  ...
  data_dir = os.path.join(FLAGS.data_dir, task_name)
  ...
  if FLAGS.do_eval and master_process:
    ...
    json_eval_file = os.path.join(FLAGS.output_dir, "eval_results.json")
    with tf.gfile.GFile(json_eval_file, "w") as fout:
      results_dict = dict([(k, float(v)) for k, v in result.items()])
  ...
  if FLAGS.do_predict and master_process:
    ...
    output_predict_file = os.path.join(FLAGS.output_dir, "dev_predictions.tsv")
    with tf.gfile.GFile(output_predict_file, "w") as writer:
        ...
        writer.write("\t".join(["example_id", "predict_class", "predict_label", "label"]))
        writer.write("\n")
        for ii, class_probs in enumerate(estimator.predict(
                input_fn=predict_input_fn, hooks=predict_hooks, yield_single_examples=True)):
            predict_class = np.argmax(class_probs.flat)
            predict_label = label_list[predict_class]
            example_id = predict_examples[ii].guid
            split, uid = example_id.split("-")
            if split == "test":
                # There is no label info.
                label = "N/A"
            else:
                label = str(predict_examples[ii].label)

            writer.write("\t".join([uid, str(predict_class), predict_label, label]))
            writer.write("\n")
    ...
    predict_examples = processor.get_mm_dev_examples(data_dir)
    predict_file = os.path.join(FLAGS.output_dir, "predict_mm.tf_record")
    file_based_convert_examples_to_features(predict_examples, label_list,
                                            FLAGS.max_seq_length, tokenizer,
                                            predict_file)
    ...
    output_predict_file = os.path.join(FLAGS.output_dir, "mm_dev_predictions.tsv")
    with tf.gfile.GFile(output_predict_file, "w") as writer:
        tf.logging.info("***** Predict results *****")
        writer.write("\t".join(["example_id", "predict_class", "predict_label", "label"]))
        writer.write("\n")
