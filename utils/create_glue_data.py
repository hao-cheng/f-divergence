...
class MnliProcessor(DataProcessor):
  ...
  def get_train_examples_w_holdout(self, data_dir, ratio=0.5):
    """See base class."""
    return self._create_examples_w_holdout(
        self._read_tsv(os.path.join(data_dir, "train.tsv")), "train",
        ratio=ratio,
    )
  ...
  def get_mm_dev_examples(self, data_dir):
    """See base class."""
    return self._create_examples(
  ...
  def _create_examples_w_holdout(self, lines, set_type, ratio=0.5):
    """Creates examples for the training and dev sets."""
    if set_type != "train":
        raise ValueError("Holdout only supports training!")
    examples = []
    num_labeled_examples = int(len(lines) * ratio)
    tf.logging.info("only keep %d labeled samples for training" % num_labeled_examples)
    for (i, line) in enumerate(lines):
      if i == 0:
        continue
...
class Sst2Processor(DataProcessor):
  ...
  def _create_examples_w_holdout(self, lines, set_type, ratio=0.5):
    ...
    if set_type != "train":
        raise ValueError("Holdout only supports training!")
    examples = []
    num_labeled_examples = int((len(lines) - 1) * ratio)
    tf.logging.info("only keep %d labeled samples for training" % num_labeled_examples)
    for (i, line) in enumerate(lines):
      if i == 0:
        continue
...
class ImdbProcessor(DataProcessor):
  ...
  def _create_examples(self, lines, set_type):
    ...
    for (i, line) in enumerate(lines):
      if i == 0:
        continue
      # Each line: label, rate, sent_id, sentence.
...
class HansProcessor(DataProcessor):
  ...
  def get_train_examples(self, data_dir):
    """See base class."""
    raise ValueError("HANS has not train!")
    return self._create_examples(
        self._read_tsv(os.path.join(data_dir, "train.tsv")), "train")
  ...
  def _create_examples(self, lines, set_type):
    ...
    for (i, line) in enumerate(lines):
      if i == 0:
        continue
      guid = "%s-%s" % (set_type, tokenization.convert_to_unicode(line[7]))
...
def convert_single_example(ex_index, example, label_list, max_seq_length,
                           tokenizer, verbose_logging=False, w_no_label=False):
  ...
  if w_no_label and example.label == "None":
    label_id = 0
    has_label = False
  else:
    has_label = True
...
def file_based_convert_examples_to_features_w_holdout(
  ...
  for (ex_index, example) in enumerate(examples):
    ...
    feature = convert_single_example(ex_index, example, label_list,
                                     max_seq_length, tokenizer, w_no_label=True)
    ...
    features["has_label"] = create_int_feature([feature.has_label])
